...
from model.output import BartSummaryOutput
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        dropout=0.0,
        bias=True,
        encoder_decoder_attention=False,  # otherwise self_attention  用于encoder中的attention
    ):
        super().__init__()
    ...
    def forward(
        ...
        # 表示encoder_decoder attention, 其中的k v全部来自encoder 隐状态
        ...
        if layer_state is not None:  # reuse k,v and encoder_padding_mask
            # dict get函数查找字典中对应键值的value,没有找到返回空字典   得到的是关于整个attention的信息
        ...
        # 当前的attention_mask指的是当前的下三角mask矩阵
...
class LearnedPositionalEmbedding(nn.Embedding):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, padding_idx: int, offset):
        ...
        super(LearnedPositionalEmbedding, self).__init__(num_embeddings, embedding_dim, padding_idx=padding_idx)
...
class BartForOnlySummaryGeneration(PretrainedBartModel):
    ...
    def forward(
        ...
        **unused,
        ...
        if not return_dict:
            output = (lm_logits,) + outputs[1:]
            return ((loss, nll_loss) + output) if loss is not None else output

        return BartSummaryOutput(
            loss=loss,
            nll_loss=nll_loss,
            logits=lm_logits,
            past_key_values=outputs.past_key_values,
